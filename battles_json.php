...
<head>
    ...
    <title>Quizztador - Batalii (JSON)</title>
    <style>
...
<body>
    <header>
        <center><h1>&#9876; Bataliile Mele — AJAX JSON (Cerința 1)</h1></center>
    </header>
    ...
    <div class="sub-nav">
        <a href="battles_json.php" class="active">1. JS + JSON</a>
    ...
    <div class="section-box">
        <h2>Afișare paginată — Vanilla JS + AJAX + JSON</h2>
        <p>Înregistrările sunt preluate câte <strong>5</strong> prin apel AJAX care returnează date în format JSON.</p>
    ...
    function loadPage(page) {
        ...
        xhr.open('GET', 'api/battles_json.php?page=' + page + '&k=' + K, true);
        ...
        xhr.onreadystatechange = function() {
            ...
            try {
                ...
                showError('Răspuns invalid de la server.');
                return;
            }
            ...
            if (data.data.length === 0) {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    ...
                    html += '<tr>';
                    html += '<td>' + b.id + '</td>';
                    html += '<td>' + escHtml(b.opponent) + '</td>';
                    html += '<td>' + escHtml(b.opponent_faction) + '</td>';
                    html += '<td>' + escHtml(b.territory) + '</td>';
                    html += '<td>' + escHtml(b.question_domain) + '</td>';
                    html += '<td>' + b.score_user + ' - ' + b.score_opponent + '</td>';
                    html += '<td class="' + resultClass(b.result) + '">' + escHtml(b.result) + '</td>';
                    html += '<td>' + b.xp_gained + '</td>';
                    html += '<td>' + escHtml(b.battle_date) + '</td>';
                    html += '</tr>';
                }
